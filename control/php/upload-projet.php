	...
	if(isset($_FILES['image']) && $_FILES['image']['error'] == 0 && isset($_POST['id'])){
		...
		if($_FILES['image']['size'] <= 20000000){
			...
			if(in_array($extension_upload, $extensions_autorisees)){
				if($id == 0){
					if(recrop($_FILES['image']['tmp_name'],'../tmp/'.$nom.'-temp.jpg','../tmp/'.$nom.'.jpg','../tmp/'.$nom.'-min.jpg',1005, 35, 35)){
						$_SESSION['images'][] = $nom;
						$_SESSION['vimeo'][] = 0;
					...
					$request = mysql_query("SELECT MAX(visuels_projet_id) AS max FROM visuels_projet");
					$val = mysql_fetch_assoc($request);
					$max = $val['max']+1;
					$request = mysql_query("SELECT MAX(visuels_projet_classement) AS classement FROM visuels_projet WHERE visuels_projet_fk = $id");
					$val = mysql_fetch_assoc($request);
					$classement = $val['classement']+1;
					$request = mysql_query("SELECT COUNT(*) AS nb FROM visuels_projet WHERE visuels_projet_fk = $id");
					$val = mysql_fetch_assoc($request);
					$nb = $val['nb'];
					if($nb == 0){
						$classement = 0;
					}
					mysql_query("INSERT INTO visuels_projet VALUES ('','projets/$id-$max.jpg','projets/$id-$max-min.jpg','0','$id','$classement')");
					mysql_close();
					if(recrop($_FILES['image']['tmp_name'],'../../projets/'.$id.'-'.$max.'-temp.jpg','../../projets/'.$id.'-'.$max.'.jpg','../../projets/'.$id.'-'.$max.'-min.jpg',1005, 35, 35)){
						echo '<script type="text/javascript">window.parent.updateFrame();</script>';
					}
				}
			}
		...
		if($_FILES['miniature']['size'] <= 20000000){
			...
			if(in_array($extension_upload, $extensions_autorisees)){
				if($id == 0){
					...
					mysql_query("INSERT INTO visuels_projet VALUES ('','projets/$id-$max.jpg','projets/$id-$max-min.jpg','$video','$id','$classement')");
					mysql_close();
					if(move_uploaded_file($_FILES['miniature']['tmp_name'],'../../projets/'.$id.'-'.$max.'-min.jpg')){
						echo '<script type="text/javascript">window.parent.updateFrame();</script>';
					}
				}
			}
